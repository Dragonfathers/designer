...
bool Tank::nextsiboom()
{
    ...
    if(tmp.m_rectSphere.left()<0||tmp.m_rectSphere.right()>WIDTH
       ||tmp.m_rectSphere.bottom()>HEIGHT||tmp.m_rectSphere.top()<0)//是否与地图边界碰撞
    {
        return true;//
    }
    return false;
...
{
    this->m_rectSphere.setRect(m_pos.x()-TANKWIDTH/2,m_pos.y()-TANKHEIGHT/2,
