...
void Bullet::Move()
{
    ...
    CalculateSphere();
    for(int i=0;i<INUM;i++)//子弹是否与地图块碰撞
    {
        for(int j=0;j<JNUM;j++)
        {
            if(glo.gamemap->getcell(i,j))
            {
                if(!glo.gamemap->getcell(i,j)->ischuantou1())
                {
                    ...
                    {
                        glo.gamemap->getcell(i,j)->downlife(wuli,0);
                        m_bDisappear=true;
                        if(group==0)
                        {
    ...
    for(int i=0;i<glo.badtanks.count();i++)//子弹是否击中敌对坦克
    {
        if(glo.badtanks.at(i)&&glo.badtanks.at(i)->group!=this->group&&IsBoom(*glo.badtanks.at(i)))
        {
            ...
            *glo.status=Status(*glo.badtanks.at(i));//将打中的对象的血量显示出来
            if(glo.badtanks.at(i)->life==0)
            {
                delete glo.badtanks.at(i);
                glo.badtanks.removeAt(i);
                i--;
            }
        }
    }
    ...
    if(m_pos.x()<0||m_pos.x()>WIDTH||m_pos.y()>HEIGHT||m_pos.y()<0)//子弹出界
    {
        m_bDisappear=true;//子弹消失
    }
}
...
{
    this->m_rectSphere.setRect(m_pos.x()-BULLETWIDTH/2,
    m_pos.y()-BULLETHEIGHT/2,BULLETWIDTH,BULLETHEIGHT);
}
